...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id == $_SESSION['user_id'] && $role !== 'admin') {
        $errors[] = 'Vous ne pouvez pas changer votre propre rôle.';
    }
    ...
    if (!$errors) {
        if ($password) {
            ...
            $conn->prepare("UPDATE users SET nom=?,prenom=?,username=?,role=?,password=? WHERE id=?")
                 ->execute([$nom, $prenom, $username, $role, $hash, $id]);
        } else {
            $conn->prepare("UPDATE users SET nom=?,prenom=?,username=?,role=? WHERE id=?")
                 ->execute([$nom, $prenom, $username, $role, $id]);
        }
        // Update session name if editing own account
  ...
  <div class="page-header">
    <div>
      <div class="text-muted small mb-1">
        <a href="index.php" class="text-decoration-none text-muted">Utilisateurs</a> / Modifier
      </div>
