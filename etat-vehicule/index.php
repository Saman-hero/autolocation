...
$params = [];
if ($typeFilter) {
    $where .= " AND ev.type = :type";
    $params[':type'] = $typeFilter;
}

$total = (int)$conn->prepare("SELECT COUNT(*) FROM etat_vehicule ev $where")->execute($params) ?
    $conn->prepare("SELECT COUNT(*) FROM etat_vehicule ev $where")->execute($params) ? (function() use ($conn, $where, $params) {
        $s = $conn->prepare("SELECT COUNT(*) FROM etat_vehicule ev $where");
        $s->execute($params);
        ...
        <table class="table table-hover mb-0">
          ...
          ?>
            <tr>
              ...
              <td>
                <a href="/location/reservations/view.php?id=<?= $e['res_id'] ?>" class="fw-semibold text-decoration-none">
