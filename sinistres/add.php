...
// Auto-référence SIN-YYYY-NNN
$year    = date('Y');
$lastRef = $conn->query("SELECT reference FROM sinistres WHERE reference LIKE 'SIN-$year-%' ORDER BY id DESC LIMIT 1")->fetchColumn();
$nextNum = 1;
if ($lastRef) { $parts = explode('-', $lastRef); $nextNum = (int)end($parts) + 1; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        if ($preResId) {
            header("Location: /location/reservations/view.php?id=$preResId");
        } else {
...
<div class="container py-4" style="max-width:680px">
  <div class="page-header">
    <div>
      <div class="text-muted small mb-1"><a href="index.php" class="text-decoration-none text-muted">Sinistres</a> / Nouveau</div>
